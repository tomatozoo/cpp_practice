...
public:
	...
	NewMarine();
	NewMarine(int x, int y, const char* marine_name);
	~NewMarine();
	int attack();
...
NewMarine::NewMarine(int x, int y, const char* marine_name) {
	coord_x = x;
	coord_y = y;
	hp = 50;
	damage = 5;
...
int mai1n() {
	//하나하나 정의하는 방식
	NewMarine marine1(2, 3,"Marine 1");
	NewMarine marine2(3, 5,"Marine 2");
	marine1.show_status();
	marine2.show_status();
	...
	//여러 인스턴스를 한번에 만드는 방식
	NewMarine* marines[100];
	marines[0] = new NewMarine(2, 3, "Marine 3");
	// 인스턴스를 동적으로 메모리에 생성 + 자동으로 생성자를 호출해준다. 
	marines[1] = new NewMarine(3, 5, "Marine 4");
